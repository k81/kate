...
func disfFlagUsages(flagSet *pflag.FlagSet) string {
	x := new(bytes.Buffer)
	...
	flagSet.VisitAll(func(flag *pflag.Flag) {
		...
		_, _ = fmt.Fprintf(x, format, shorthand, flag.Name, flag.DefValue, flag.Usage)
	})
...
func UsageFunc(cmd *cobra.Command) error {
	...
	data := &UsageData{
		Cmd:         cmd,
		LocalFlags:  disfFlagUsages(cmd.LocalFlags()),
		GlobalFlags: disfFlagUsages(cmd.InheritedFlags()),
		SubCommands: subCommands,
	}
	_ = commandUsageTemplate.Execute(tabOut, data)
	_ = tabOut.Flush()
	return nil
}
